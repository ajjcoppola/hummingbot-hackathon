...
| Mon 24 | Fresh stack | v2.16 Docker + Gateway up; `gateway connect solana` on **devnet** |
| Tue 25 | First on-chain loop | Devnet open → out-of-range close → reopen; log tx hashes |
| Wed 26 | Policy on chain | Custom controller or documented `lp_rebalancer` fallback running |
...
Out of scope until after freeze: Coinbase V2 upgrade, zipline↔live MQTT, IB hybrid, Condor LLM order placement.
